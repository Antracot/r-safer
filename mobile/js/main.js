...
var mob_height = $(window).height();
// console.log(mob_height);
var mob_width = $(window).width();
// console.log(mob_width);
$('.wrap').css('width', mob_width).css('height', mob_height);

if(mob_height<mob_width){
}
...
function first_kadr_animate(){
	...
	firstTL.to($("#line"), 1,{width: mob_width})
		.to($('.slash'), .5, {alpha:1})
		.to($('#f_slash'), .3, {x:-35})
		.to($('#s_slash'), .3, {x:30, delay:-.3})

        .to($('#c_lang'), .4, {top:-5, delay:-.2})
		.to($('#e_lang'), .4, {left:8, delay:-.4})
		.to($('#r_lang'), .4, {right:5, delay:-.4})
...
//     	annot_active = true;
//     	slideAlpha = true;
//     	var videMove = new TweenMax.to($('.background-wrap'), 0.01, {y:50, alpha:0, onComplete: function(){
...
$(".lang a").on("click", function(e) {
        ...
        if(annot_active){
        	...
        	var first_closeTL = new TimelineMax();
        		...
        		first_closeTL.to($('.logos'), 1, {top:-12, scale: 0.55, y: -5})
        				.to($('.footer'), 1, {bottom:-50, scale: 0.7, delay:-1})
        				...
        				.to($('#slide2'), 1, {alpha:1, delay: -0.5, onComplete: function(){
        				// $('.play_btn').show();
                        // $('#slide2').show();
        				}})
        				.to($('.lang'), 0.5, {x:kw, y:kh, scale:0.7})
        				.to($('.lang'), 0.5, {alpha:1})

            setTimeout(function(){ 
                    $("#r_arrow").trigger("click").trigger("click"); 
                    $('.arrow').show();  
                }, 1000);
...
$('.wrap').css('width', mob_width).css('height', mob_height);
    ...
    if(mob_height<mob_width){
    console.log("FUCK");
        $('.popup_news .details_big .img_big').css('margin', '7% auto');
    
    }
    else{
        $('.popup_news .details_big .img_big').css('margin', '45% auto');
    }
...
function moveSlide(){
	...
	$('.blur_win').css('width', sh_podl5*0.8+'px').css('height', w_podl5*0.8+'px')
    // .css('top', topPos+'px')
    ;
	...
	$('.blur_win').css('width', widthScr_win+'px').css('height', heightScr).css('margin-left', -widthScr_win/2+'px').css('left','50%')
        ...
        var pageTL = new TimelineMax();
                    pageTL
                        ...
                        .fromTo($('#slide'+active_slide).find('.line_gorizontal'),.4, {width:0}, {width: 300, delay: -0.2, ease:Linear.easeNone})
...
// $(function() {
//     $('#scroll_win').touchScrolling();
// });

// $('.scroll_window_wrap').on("touchdown", function(){
//     console.log("popup");
// });
